import { CommonModule } from '@angular/common';
import { Component, OnInit } from '@angular/core';

declare const PDFViewCtrl: any;

@Component({
  selector: 'app-pdf-viewer',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './pdf-viewer.component.html',
  styleUrls: ['./pdf-viewer.component.scss'],
})

// declare interface PDFViewCtrl{
//   PDFViewer : any
// };
export class PdfViewerComponent implements OnInit {
  private _pdfViewer: any;

  ngOnInit(): void {
    // this.initializePDFViewer();
  }

  // initializePDFViewer(): void {
  //   const { PDFViewer } = PDFViewCtrl;
  //   console.log(PDFViewCtrl);

  //   this._pdfViewer = new PDFViewer({
  //     libPath: './lib', // path to Web SDK library
  //     jr: {
  //       licenseKey:
  //         '',
  //       licenseSN: 'eCwtLet2zR57snNdu2ZZcwwi8rs8B61fcvofaZLjWW7i6uj/TSlNtw==',
  //     },
  //   });
  //   this._pdfViewer.init('#foxitframe');
  // }

  // initializeIframe():void{

  //       var { PDFViewer } = PDFViewCtrl;

  //       // Initialize PDF Viewer
  //       var pdfViewer = new PDFViewer({
  //         libPath: 'public/lib', // Ensure this path is correct
  //         jr: {
  //           licenseKey:
  //             '',
  //           licenseSN:
  //             'eCwtLet2zR57snNdu2ZZcwwi8rs8B61fcvofaZLjWW7i6uj/TSlNtw==',
  //         },
  //       });

  //       pdfViewer.init('#pdf-viewer'); // Initialize the viewer

  //       // File upload handling
  //       // document
  //       //   .getElementById('pdf-upload')
  //       //   .addEventListener('change', function (event) {
  //       //     var file = event.target.files[0];
  //       //     if (file && file.type === 'application/pdf') {
  //       //       var reader = new FileReader();
  //       //       reader.onload = function (e) {
  //       //         var buffer = e.target.result;
  //       //         console.log('PDF buffer ready to send to iframe:', buffer);

  //       //         // Send the PDF buffer to the iframe
  //       //         var iframe = document.getElementById('pdf-viewer');
  //       //         iframe.contentWindow.postMessage(buffer, '*');
  //       //       };
  //       //       reader.readAsArrayBuffer(file); // Read the file as an ArrayBuffer
  //       //     } else {
  //       //       alert('Please upload a valid PDF file.');
  //       //     }
  //       //   });
  // }

  // onFileUpload(event: Event): void {
  //   const input = event.target as HTMLInputElement;
  //   // const file = input?.files?.[0];

  //   const file = input?.files?.[0];

  //   if (file && file.type === 'application/pdf') {
  //     const reader = new FileReader();
  //     reader.onload = (e) => {
  //       const buffer = e.target?.result;
  //       console.log('PDF buffer ready to send to iframe:', buffer);

  //       // Send the PDF buffer to the iframe via postMessage
  //       const iframe = document.getElementById('pdf-iframe') as HTMLIFrameElement;
  //       iframe.contentWindow?.postMessage(buffer, '*');
  //     };
  //     reader.readAsArrayBuffer(file);
  //   } else {
  //     alert('Please upload a valid PDF file.');
  //   }



  //   // if (file && file.type === 'application/pdf') {
  //   //   const reader = new FileReader();
  //   //   reader.onload = (e) => {
  //   //     const buffer = e.target!.result;
  //   //     this._pdfViewer.openPDFByFile(buffer); // Open the uploaded PDF file
  //   //   };
  //   //   reader.readAsArrayBuffer(file);
  //   // } else {
  //   //   alert('Please upload a valid PDF file.');
  //   // }
  // }
}
